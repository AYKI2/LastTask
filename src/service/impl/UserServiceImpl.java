...
public class UserServiceImpl implements UserService {
    ...
    public User findUserById(Long id) {
        List<User> users = userList.stream().filter(x -> x.getId().equals(id)).toList();
        User user1 = new User();
        for (User user : users) {
            user1 = user;
            break;
    ...
    public String buyBooks(String name, List<Book> books) {
        ...
        for (User user:userList) {
            if (user.getId().equals(id)) {
                ...
                for (Book book : books) {
                    if (book.getName().equals(name) && user.getMoney().intValue() >= book.getPrice().intValue()) {
                        user.getBooks().add(book);
                        int money = user.getMoney().intValue() - book.getPrice().intValue();
                        user.setMoney(BigDecimal.valueOf(money));
                        String finalName = name;
                        books.removeIf(x -> x.getName().equals(finalName));
